...
function parseString(input, tokenList, index, settings) {
	const token = tokenList[index];
	const string = input.slice(token.loc.start.offset + 1, token.loc.end.offset - 1);
	...
	for (let i = 0; i < string.length; i ++) {
		...
		if (char === '\\') {
			...
			const column = token.loc.start.column + i + 1;
			error(
		...
		switch (state) {
			case propertyStates._START_: {
				if (token.type === tokenTypes.STRING) {
					const key = {
						type: 'Identifier',
						value: parseString(input, tokenList, index, settings),
						raw: token.value
					};
					if (settings.loc) {
						key.loc = token.loc;
